...
- 支持按固定金额或当前资产百分比投入 / 拿走、多段现金流计划、价值曲线与参数缓存的复利计算器
...
每条计划可选择“投入 / 拿走”和“固定金额 / 当前资产百分比”，再设置首次执行时间、持续月数和周期。百分比按每次执行前的资产计算，包含到该时点已累计的收益；同月多条计划使用同一个基数。

例如本金 100,000、年利率 0%，从第 1 个月开始每月拿走 2%：第 1 个月拿走 2,000，余额 98,000；第 2 个月拿走 1,960，余额 96,040。百分比投入则表示从外部追加相应资金。原有固定金额参数会自动恢复；切换计量方式后需重新填写数值。

在价值曲线上移动鼠标或拖动时间滑块，下方会显示所选月份的投入、取走、占执行前资产的比例、净流入、收益和各条实际执行的计划。移开鼠标后仍保留该月，方便阅读。起始时点单独展示，最后不足一个月时只计算到投资终点。
